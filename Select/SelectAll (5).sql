	...
	begin 
		...
		begin
			...
			begin transaction
				...
				(ID, SaleDate, ProductID, Quantity) 
				values 
				((
				  select max(id) + 1
				  from ProductSale), GETDATE(), (
												 select id 
												 from Product
												 where Title = @TitleOfProduct), @CountOfProduct);
				print 'Сведения о продаже товара ' + @TitleOfProduct + ' в количестве ' + Cast(@CountOfProduct AS NVARCHAR(12)) + ' на сумму ' + Cast(@SumSale AS NVARCHAR(12)) + ' добавлены в базу данных';
			commit transaction
			print 'Transaction committed'
			end try
