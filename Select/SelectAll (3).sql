...
alter proc PR_ClientServiceList	@TitleService nvarchar(100),	@StartTimeService datetime,	@PhoneClient nvarchar(20)as	if not exists (
	    ...
	    begin
			...
			values(@IdClient, @IdService, @StartTimeService);
...
alter proc PR_SalesOnDate
	@firstDate date,
	@secondDate date = null
as
	if (@secondDate = null)
		begin
			set @secondDate = dateadd(day, 1, @firstDate);
			return;
		end
	if not exists(
				  select * from ProductSale
...
exec PR_SalesOnDate '09.02.2019', '05.03.2019'
